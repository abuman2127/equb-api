...
app.get('/winners', requireLogin, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            `SELECT po.id, po.round_number AS round, m.full_name AS winner_name, po.amount, po.payout_date AS date_won
             FROM payouts po
...
app.delete('/payouts/:id', requireLogin, requireAdmin, async (req, res) => {
    const { id } = req.params;
    try {
        const result = await pool.query(
            `DELETE FROM payouts WHERE id = $1 RETURNING *`,
            [id]
        );
        if (result.rows.length === 0) {
